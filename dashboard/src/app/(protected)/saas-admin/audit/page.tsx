...
const fetcher = () => saasAdminApi.getAuditLogs().then((r) => r.data);

export default function SaasAuditLogsPage() {
  const { user } = useAuthStore();
  
  // Only super_admin can view global SaaS audit logs
  const isSuperAdmin = user?.role === 'super_admin';
  const { data, isLoading } = useSWR(isSuperAdmin ? 'saas-audit-logs' : null, fetcher);
  const logs: any[] = data ?? [];
  ...
  if (!isSuperAdmin) {
    return (
      <div className="empty-state">
        ...
        <p className="empty-state-text">Access Denied. Only Super Admins can view Global Audit Logs.</p>
      </div>
    );
  }
  ...
  const getActionColor = (action: string) => {
    if (action.includes('UPDATE') || action.includes('RESET') || action.includes('RESTORE')) return '#0ea5e9';
    if (action.includes('BLOCK') || action.includes('MANUAL')) return '#f59e0b';
    if (action.includes('CREATE') || action.includes('ADD') || action.includes('PUBLISH') || action.includes('ONBOARD')) return '#10b981';
    if (action.includes('DELETE') || action.includes('REMOVE') || action.includes('ARCHIVE')) return '#ef4444';
    return '#8b5cf6';
  };

  const getModuleIcon = (module: string) => {
    switch (module.toUpperCase()) {
      case 'SAAS_ADMIN': return <ShieldCheck size={14} />;
      ...
      <div className="table-wrap" style={{ border: 'none', background: 'transparent' }}>
        {isLoading ? (
          ...
          <div className="empty-state" style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', minHeight: 400 }}>
            ...
            <p className="empty-state-text">No global audit logs match your criteria.</p>
          </div>
        ) : (
