    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="empty-state">
          ...
          <p className="empty-state-text">No global calendar templates defined yet. Onboarded schools will start without a calendar.</p>
        </div>
      ) : (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    {yearTerms.length < 3 && (
                      ...
                      >
                        <Plus size={14} /> Add Term Template
                      </button>
                    )}
                            ...
                            <div key={term.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 16 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                ...
                                <div style={{ display: 'flex', gap: 4 }}>
                                  <button 
                                    ...
                                    aria-label="Edit Term Template"
                                    title="Edit Term Template"
                                  >
                                    <Edit2 size={14} />
                                  </button>
                                  <button 
                              ...
                              <div style={{ background: 'rgba(255,255,255,0.03)', padding: '8px 12px', borderRadius: 6, fontSize: 12 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                                  <span style={{ color: 'var(--text-secondary)' }}>Standard Starts:</span>
                                  <span style={{ fontWeight: 600 }}>{format(parseISO(term.startDate), 'MMM do, yyyy')}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                  <span style={{ color: 'var(--text-secondary)' }}>Standard Ends:</span>
                              ...
                              <div>
                                ...
                                {term.breaks?.length === 0 ? (
                                  <p style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'center', padding: '6px 0', margin: 0 }}>
                                    No break templates scheduled for this term.
                                  </p>
                                ) : (
        ...
        <div className="modal-overlay" onClick={() => setShowYearModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 600 }}>
            <div className="modal-header">
              <h3>Create Academic Year Template</h3>
            ...
            <form onSubmit={handleYearSubmit}>
              ...
              <div className="modal-footer" style={{ marginTop: 24 }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={isSubmitting}>
                  {isSubmitting ? 'Saving...' : 'Save Year Template'}
                </button>
              </div>
            </form>
          </div>
      ...
      {showTermModal && (
        <div className="modal-overlay" onClick={() => setShowTermModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{editingTerm ? 'Edit Term Template' : 'Add Term Template'}</h3>
            ...
            <form onSubmit={handleTermSubmit}>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={isSubmitting}>
                  {isSubmitting ? 'Saving...' : 'Save Term Template'}
                </button>
              </div>
            </form>
          </div>
      ...
      {showBreakModal && (
        <div className="modal-overlay" onClick={() => setShowBreakModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 450 }}>
            <div className="modal-header">
              <h3>Add Mid-term Break Template</h3>
            ...
            <form onSubmit={handleBreakSubmit}>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={isSubmitting}>
                  {isSubmitting ? 'Saving...' : 'Add Break Template'}
                </button>
              </div>
            </form>
          </div>
