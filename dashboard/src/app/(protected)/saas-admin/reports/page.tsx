...
async function generateSchoolsPdf(
  ...
  doc.save(`schools-performance-term-${isoDate}.pdf`);
}
...
async function generateEmployeesPdf(
  ...
  const tableBody = employees.map((emp, i) => {
    ...
    const signouts = emp.metrics.forgotOutRate ?? 0;
  ...
  autoTable(doc, {
    startY: firstPageTableY,
    head: [['#', 'Employee Name', 'Code', 'School', 'Presence', 'Punctuality', 'Hours', 'Sign-outs', 'Score']],
    body: tableBody,
    theme: 'grid',
    styles: {
      fontSize: 8,
    ...
    columnStyles: {
      ...
      1: { cellWidth: 'auto' as any, minCellWidth: 55 },
  ...
  doc.save(`employee-rankings-term-${sortOrder}-${isoDate}.pdf`);
}
...
async function generateSummaryPdf(
  ...
  doc.save(`platform-summary-term-${isoDate}.pdf`);
}
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
        <thead>
          <tr style={{ borderBottom: '1px solid var(--border)' }}>
            {['#', 'Employee', 'Code', 'School', 'Presence', 'Punctuality', 'Hours', 'Sign-outs', 'Score'].map((h) => (
              <th
                key={h}
                style={{
                  padding: '10px 14px',
                  ...
                  letterSpacing: '0.05em',
                  whiteSpace: 'nowrap',
                  textAlign: ['#', 'Code', 'Presence', 'Punctuality', 'Hours', 'Sign-outs', 'Score'].includes(h) ? 'center' : 'left',
                }}
              >
                {h}
              </th>
...
export default function ReportsPage() {
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      const timestamp = format(now, "MMMM d, yyyy 'at' hh:mm a");
