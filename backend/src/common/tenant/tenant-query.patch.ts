...
export function patchTypeORMRepository() {
  ...
  const applyTenantFilter = (metadata: any, options: any, tenantId: string) => {
    ...
    const hasTenantId = metadata.columns.some((col: any) => col.propertyName === 'tenantId');
    ...
    if (options.where) {
      if (Array.isArray(options.where)) {
        if (options.where.some((w: any) => 'tenantId' in w)) {
          console.log(`📌 [TypeORM Patch] Bypassed auto-inject for ${metadata.name} (explicit tenantId found in array where)`);
          return options;
        }
      } else {
        if ('tenantId' in options.where) {
          console.log(`📌 [TypeORM Patch] Bypassed auto-inject for ${metadata.name} (explicit tenantId found in object where)`);
          return options;
        }
      }
    }
    ...
    console.log(`📌 [TypeORM Patch] Auto-injected tenantId filter for ${metadata.name}:`, options.where);
    return options;
  };
  ...
  Repository.prototype.find = function (options: any = {}) {
    ...
    console.log(`🔍 [TypeORM Patch] Repository.find() called for ${metadata.name} | Active Tenant ID: ${tenantId}`);
  ...
  Repository.prototype.findOne = function (options: any = {}) {
    ...
    console.log(`🔍 [TypeORM Patch] Repository.findOne() called for ${metadata.name} | Active Tenant ID: ${tenantId}`);
  ...
  Repository.prototype.findAndCount = function (options: any = {}) {
    ...
    console.log(`🔍 [TypeORM Patch] Repository.findAndCount() called for ${metadata.name} | Active Tenant ID: ${tenantId}`);
  ...
  Repository.prototype.count = function (options: any = {}) {
    ...
    console.log(`🔍 [TypeORM Patch] Repository.count() called for ${metadata.name} | Active Tenant ID: ${tenantId}`);
      ...
      const injectTenant = () => {
        if (!(qb as any).__tenantInjected) {
          ...
          const hasTenantCondition = qb.expressionMap.wheres.some(
            (w: any) => w.condition && (w.condition.includes('tenantId') || w.condition.includes('tenant_id'))
          );

          if (!hasTenantCondition) {
            qb.andWhere(`${alias}.tenantId = :tenantId_auto`, { tenantId_auto: tenantId });
            console.log(`📌 [TypeORM Patch] Auto-injected tenantId filter for queryBuilder on ${metadata.name} (alias: ${alias}): ${tenantId}`);
          } else {
            console.log(`📌 [TypeORM Patch] Bypassed auto-inject for queryBuilder on ${metadata.name} (alias: ${alias}) due to explicit condition.`);
          }
          (qb as any).__tenantInjected = true;
        }
      };

      const originalGetMany = qb.getMany;
      qb.getMany = function () { injectTenant(); return originalGetMany.call(this); };

      const originalGetOne = qb.getOne;
      qb.getOne = function () { injectTenant(); return originalGetOne.call(this); };

      const originalGetCount = qb.getCount;
      qb.getCount = function () { injectTenant(); return originalGetCount.call(this); };

      const originalGetManyAndCount = qb.getManyAndCount;
      qb.getManyAndCount = function () { injectTenant(); return originalGetManyAndCount.call(this); };

      const originalGetRawMany = qb.getRawMany;
      qb.getRawMany = function () { injectTenant(); return originalGetRawMany.call(this); };

      const originalGetRawOne = qb.getRawOne;
      qb.getRawOne = function () { injectTenant(); return originalGetRawOne.call(this); };
    }
